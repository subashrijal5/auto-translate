...
class ApiClient {
     ...
     */
    async getLanguageStrings(language) {
        const languageUrl = new URL(this.baseUrl + "/api/projects/languages/" + language + "/strings");
        languageUrl.searchParams.append("grouped", config_1.config.GROUPED);
        const response = await fetch(languageUrl, {
            method: "GET",
            headers: {
                "X-PROJECT-KEY": this.token,
